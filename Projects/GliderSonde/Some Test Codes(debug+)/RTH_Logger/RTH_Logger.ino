...
boolean Sd_makenewFile(){
  ...
  if (!file.createContiguous(fileName, 512UL*BLOCK_COUNT)) return false; //file~~
...
void setup() {
  ...
  if(!Gyro_init()||!Acc_init()||!Sd_init()||!Sd_makenewFile()) while(1){delay(1000);}
  ...
  if(loop_counter%Output_step==0){//Output pulse.
    ...
    if(Sd_whileWriting && Sd_Stack>=8){
      if(Sd_Write()){
        ...
        if(++curBlock>=BLOCK_COUNT){if(!Sd_Stop()) Sd_Error = true; Sd_whileWriting = false;}
      }
      else{//Sd_Write failed
        if(!Sd_Stop()) Sd_Error = true;
      }
    }
    
    else if(Sd_whileWriting && PPM_Input[2] < 1050 && PPM_Input[3] < 1050){//Does User want to STOP recording?
      Sd_Stack=0; if(!Sd_Stop()){Sd_Error = true;} Sd_whileWriting = false;
    }
    
    else if(!Sd_whileWriting && PPM_Input[2] > 1950 && PPM_Input[3] > 1950){//Deos User want to START recording?
      Sd_Stack = 0; newPointer = (unsigned char*)pCache; curBlock = 0;
      if(!Sd_makenewFile()) Sd_Error = true;
    }
    
  }//Sd card processing part....
