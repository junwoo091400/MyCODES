...
#include "FreeStack.h"
void COUNTER_FUCKER(){
  Serial.println("I am COUNTER_FUCKER");
  for(int i=5;i>=0;i--){
    Serial.println(i);
    delay(600);//approx total 3 sec.
  }
}
// SD chip select pin
...
const uint32_t TEST_TIME_SEC = 10; // 10 Seconds!

// Time between printing progress dots
const uint32_t DOT_TIME_MS = 5000UL;

// number of blocks in the contiguous file
const uint32_t BLOCK_COUNT = (1000*RATE_KB_PER_SEC*TEST_TIME_SEC + 511)/512; //  1954 쯤!
...
void loop(void) {
  ...
  while(Serial.available()){Serial.read();}
  cout << F("Will create Contiguous file\n");
  while(!Serial.available()){delay(100);}
  COUNTER_FUCKER();
  cout << F("file.createContiguous() !\n");
  ...
  while(Serial.available()){Serial.read();}
  cout << F("Will get Range of blocks!\n");
  while(!Serial.available()){delay(100);}
  COUNTER_FUCKER();
  cout << F("file.contiguousRange!\n");
  ...
  for(int i=0;i<26;i++) pCache[i+6] = 65+i;//'A'(0x41)~'Z'(0x5A)

  cout << F("Start raw write of ") << file.fileSize()/1024UL << F(" KB\n");
  cout << F("Target rate: ") << RATE_KB_PER_SEC << F(" KB/sec\n");
  cout << F("Target time: ") << TEST_TIME_SEC << F(" seconds\n");
  
  // tell card to setup for multiple block write with pre-erase
  while(Serial.available()){Serial.read();}
  cout << F("Will writeStart!\n");
  while(!Serial.available()){delay(100);}
  COUNTER_FUCKER();
  cout << F("sd.card()->writeStart!\n");
  ...
  while (nWrite < BLOCK_COUNT) {
    ...
    while(Serial.available()){Serial.read();}
  cout << F("Will writeData!\n");
  while(!Serial.available()){delay(100);}
  COUNTER_FUCKER();
  cout << F("sd.card()->writeData!\n");
    uint32_t tw = micros();
    ...
    nWrite++;
    cout << ((float)nWrite/BLOCK_COUNT) << F("% done\n");
    if(nWrite==5) Serial.println("Done!"); break;
  }
