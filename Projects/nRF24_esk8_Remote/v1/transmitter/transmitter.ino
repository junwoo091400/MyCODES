...
#define ResetEEPROM_settingIdx (numOfSettings - 1)
...
int settingRules[numOfSettings][3] {
  ...
  {16, 0, 250}, // Motor pulley
  {33, 0, 250},// Wheel pulley
  {83, 0, 250},//Wheel diameter.
  {1, 0, 1}, // Yes or no
  {360, 0, 1023},
  {500, 0, 1023},//SUCH a small Range. ... :(
  {680, 0, 1023},
  {-1,0,0}
};
...
radioCommand command_transmit;

void radioComamnd_calculateCRC(radioCommand *rc)
{
  uint16_t calculated_crc = crc16((unsigned char*)rc, sizeof(radioCommand) - 2);//exclude last crc.
  rc -> crc = calculated_crc;
}
//
...
void setup() {
  ...
  radio.openWritingPipe(pipe);
    ...
    radio.printDetails();
  #endif
  randomSeed(analogRead(A0));//for NOw. it is unconnected. Right??? lol.
}
...
void loop() {
  ...
  else
  {
    ...
    if (!triggerActive()) {
      ...
      command_transmit.motorSpeed = 127;
    }
...
bool isInDeadband(int value, int deadband)
{
  return (abs(value) <= deadband) ? (true) : (false);
}

float getThrottlePosition()
{
  float returnVal = 0.0f;
  if( isInDeadband(hallMeasurement - remoteSettings.centerHallValue, 5)) {
    return 0.0f;
  }
  else if( hallMeasurement > remoteSettings.centerHallValue) {//High
    float numerator = hallMeasurement - remoteSettings.centerHallValue;
    float denominator = remoteSettings.maxHallValue - remoteSettings.centerHallValue;
    returnVal = numerator / denominator;
  }
  else{//LOW.
    float numerator = hallMeasurement - remoteSettings.centerHallValue;
    float denominator = remoteSettings.centerHallValue - remoteSettings.minHallValue;
    returnVal = numerator / denominator;
  }
  return constrain(returnVal, -1.0f, 1.0f);
}
...
void controlSettingsMenu() {
  if (triggerActive()) {
    if (settingsChangeFlag == false) {
      ...
      if (changeSelectedSetting == true) {
        if(currentSetting == ResetEEPROM_settingIdx) {//////////////SPECIAL CASE, useful tho.
          drawTitleScreen("EEPROM Init");
  ...
  if ( (throttlePos > 0.5) && settingsLoopFlag == false) {//hallMeasurement >= (remoteSettings.maxHallValue - 150)
    // Up
    if (changeSelectedSetting == true) {
  ...
  else if ( (throttlePos < -0.5) && settingsLoopFlag == false) {//hallMeasurement <= (remoteSettings.minHallValue + 150)
    // Down
    if (changeSelectedSetting == true) {
      int val = getSettingValue(currentSetting) - 1;
  ...
  } else if (throttlePos == 0.0) {//inRange(hallMeasurement, remoteSettings.centerHallValue - 50, remoteSettings.centerHallValue + 50)) {
...
void loadEEPROMSettings() {
  ...
  #ifdef DEBUG
  DEBUG_PRINT("Got settings from EEPROM");
  for(int i=0;i<numOfSettings;i++){
    DEBUG_PRINT(getSettingValue(i));
  }
  DEBUG_PRINT("That's All I've Got");
  #endif
...
void transmitToVesc() {
  ...
  if (millis() - lastTransmission >= 50) {
    ...
    for(int i=0;i<3;i++) {
      command_transmit.extraChannel[i] = random(0xFFFF);//uint16_t man.
    }
    radioComamnd_calculateCRC(&command_transmit);//calculate CRC.
    sendSuccess = radio.write(&command_transmit, sizeof(radioCommand));
...
void calculateThrottlePosition() {
  ...
  if (hallMeasurement >= remoteSettings.centerHallValue) {
    command_transmit.motorSpeed = constrain(map(hallMeasurement, remoteSettings.centerHallValue, remoteSettings.maxHallValue, 127, 255), 127, 255);
  } else {
    command_transmit.motorSpeed = constrain(map(hallMeasurement, remoteSettings.minHallValue, remoteSettings.centerHallValue, 0, 127), 0, 127);
  }

  // removeing center noise
  if (abs(command_transmit.motorSpeed - 127) < hallCenterMargin) {
    command_transmit.motorSpeed = 127;
  }
}
...
void drawThrottle() {
  ...
  if (command_transmit.motorSpeed >= 127) {
    int width = map(command_transmit.motorSpeed, 127, 255, 0, 49);
    ...
    int width = map(command_transmit.motorSpeed, 0, 126, 49, 0);
    for (int i = 0; i < width; i++) {
      //if( (i % 2) == 0){
